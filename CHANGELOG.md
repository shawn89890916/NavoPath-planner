...
### 修复
- 全局 AI 现在会区分登录失效、工作区故障、额度限制、临时网络或上游错误；登录令牌会自动刷新一次，异常的历史对话或记忆数据会安全降级，短暂的供应商 5xx 与网络失败会自动重试一次，并忽略外部日历读取故障，避免它阻断工作区 Agent。
