...
- 今日候选快速添加按钮改为 Add，加粗放大并响应手形悬停；浅色使用白底黑字、深色使用黑底白字，禁用时保留填充并明确显示禁用状态。
- 禁用的快速添加按钮在悬停和按下时保持静止，避免产生仍可操作的动效反馈。
- 深色模式左上角产品图标使用反色并保留原有明暗细节，避免变成整块纯白；快速添加 ADD 按钮加宽并采用与界面一致的 squircle 圆角。
...
- 普通与逾期候选任务均可在安排入口悬停约 300 毫秒后预览建议时段，并从右侧“定位/返回”平滑聚焦时间轴或恢复原位置；未定位的预览在指针离开任务卡后收起。
...
- 竖屏设置详情移除重复的分类下拉列表，基础信息直接上移；返回设置主页后仍可切换其他设置分区。
...
- 设置分类、设置内页标签与 Planning 视图切换统一采用整行圆角选中态，替换旧的底边半包样式；通知中心、全局搜索及后续浅色界面统一继承时间轴的中性纸面底色，避免出现泛紫表面。
- 降低网页端同步造成的卡顿与掉线：线上网页停止建立不稳定的跨域 Realtime WebSocket，改用切回页面时立即检查、前台每 30 秒轻量检查版本；连续编辑合并为 750 毫秒后保存，减少整份工作区的重复上传与重连压力。
...
- Changed the Today’s Candidates quick-add button to Add with bold type and a pointer cursor on hover. It uses white with black text in light mode and black with white text in dark mode, while retaining its fill and clearly showing its disabled state.
- Disabled quick-add buttons stay still on hover and press, avoiding motion that suggests they can be used.
- Inverted the top-left product mark in dark mode while preserving its light and dark details. The quick-add button now reads Add in bold type, shows a pointer cursor, and uses white with black text in light mode and black with white text in dark mode.
...
- Regular and overdue candidates now preview suggested times after about 300 ms of hover. The right-side Locate/Back action smoothly focuses the timeline or restores its previous position; an unlocated preview closes when the pointer leaves the task card.
...
- Removed the duplicate category selector from portrait settings details so the information starts higher; users can return to the Settings home to switch sections.
...
- Settings categories, Settings tabs, and Planning view controls now share a complete rounded-row selection state instead of the old partial underline. The notification center, global search, and future light-mode interfaces inherit the timeline's neutral paper surface to prevent a lavender cast.
- Reduced web sync lag and disconnect churn: production web no longer opens an unreliable cross-origin Realtime WebSocket, using an immediate foreground check plus a lightweight 30-second revision check instead. Continuous edits are coalesced for 750 ms before saving to avoid repeated full-workspace uploads.
