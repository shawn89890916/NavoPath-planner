    ...
    <section className="df-proactive-center" role="dialog" aria-modal="true" aria-labelledby="df-proactive-center-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="df-proactive-center-list">
        ...
        {notifications.map((item) => <article key={item.id} className="df-proactive-center-item">
          <CloseButton className="df-proactive-center-item-dismiss" onClick={() => onDismiss(item)} label={lang === "zh" ? `关闭${item.title}` : `Dismiss ${item.title}`} />
          ...
          {item.kind === "unfinished_tasks" && <button type="button" className="df-proactive-center-action" onClick={() => onOpenUnfinished(item)}>{lang === "zh" ? "确认未完成任务" : "Review incomplete tasks"}</button>}
